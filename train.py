...
def train_gssoft(args):
    ...
    training_dataset = CustomDataset(size=(image_size, image_size), train=True)
    test_dataset = CustomDataset(size=(image_size, image_size), train=True)
...
def train_vqvae(args):
    ...
    training_dataset = CustomDataset(size=(image_size, image_size), train=True)
    test_dataset = CustomDataset(size=(image_size, image_size), train=False)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for i, images in enumerate(tqdm(training_dataloader), 1):
            images = images.to(device)
            ...
            if global_step % len(training_dataloader) == 0:
                save_checkpoint(model, optimizer, global_step, checkpoint_dir)
        ...
        model.eval()
        average_logp = average_vq_loss = average_elbo = average_bpd = average_perplexity = 0
        for i, images in enumerate(test_dataloader, 1):
            images = images.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", choices=["VQVAE", "GSSOFT"], default="VQVAE", help="Select model to train (either VQVAE or GSSOFT)")
